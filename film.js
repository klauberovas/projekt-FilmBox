...
let hledanyFilm = null;

//Najít film podle shody s Id z předešlé stránky s id filmu na současné stránce
filmy.forEach((film) => {
  if (film.id === filmId) {
    hledanyFilm = film;
  }
...
detailFilmuElm.querySelector('.card-text').textContent = hledanyFilm.popis;
detailFilmuElm.querySelector('.card-title').textContent = hledanyFilm.nazev;
detailFilmuElm.querySelector('img').src = `${hledanyFilm.plakat.url}`;

//Formátování času

const now = dayjs();
const premiereDate = dayjs(hledanyFilm.premiera);

detailFilmuElm.querySelector('#premiera').innerHTML = `
Premiéra <strong>${dayjs(hledanyFilm.premiera).format(
  'D.M.YYYY',
